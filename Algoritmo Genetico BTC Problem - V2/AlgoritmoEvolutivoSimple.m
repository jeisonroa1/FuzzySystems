...
precioN=(precio(1:320)-min(precio(1:320)))/(max(precio(1:320))-min(precio(1:320)));%Normalización de datos (Scaling).
meanp=mean(precioN);
...
f=0;      %No. de Individuos para elitismo  DESACTIVADO Comentarios sección elitismo
%% Variables
...
for i =1:npop,
    ...
    fobj(i,1)=FISerror(fis,precioNM,n,H); % Cálculo del error del fenotipo i 
end;
fobjMax = max(fobj);
fobjMin = min(fobj);
fobjAv  = mean (fobj);
...
%% Elitismo (Editar sección: Nueva población)
% orden = sort(fobj);
% for t=1:f,
